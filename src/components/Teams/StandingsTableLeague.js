...
function StandingsTableLeague({standings}) {
    ...
    const leagueStandings = formatStandingData(standings, "League");
         ...
         <table>
            <thead>
                ...
                <tbody>
                    ...
                    <tr key={t.team.name}>
                        <td>{t.leagueRank}    <img className="Standings-logo" src={`https://www-league.nhlstatic.com/images/logos/teams-current-primary-light/${t.team.id}.svg`} style={{width: '30px', height: '30px'}} alt={`logo-${t.team.name}`} /> {t.clinchIndicator ? `${t.clinchIndicator}-` : ""}<Link key={t.team.id} to={`/teams/${t.team.id}`}>{t.team.name}</Link></td>
                        <td>{t.gamesPlayed}</td>
...
export default StandingsTableLeague;